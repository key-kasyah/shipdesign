            ...
            {activeTab === "geometry" && (
              <div className="space-y-6 max-w-7xl mx-auto">
                ...
                <div className="bg-slate-900/60 border border-slate-800/80 p-6 rounded-2xl space-y-6 backdrop-blur-xl shadow-2xl">
                  ...
                  <div className="bg-slate-950/80 p-6 rounded-2xl border border-slate-800/80 space-y-6 backdrop-blur-md shadow-inner">
                    {/* CSA SVG Chart */}
                    {/* CSA SVG Chart */}
                    <div className="space-y-2">
                      {(() => {
                        const activeAm = (Number(designData.breadth_m) || 14) * (Number(designData.draft_m) || 5) * (Number(designData.cm) || 0.98);
                        const csaOrds = (designData.geometry?.csa_ordinates && designData.geometry.csa_ordinates.length === 21)
                          ? designData.geometry.csa_ordinates
                          : Array.from({ length: 21 }).map((_, i) => Number(((getStationOrdinate(i, designData.cb || 0.76) / 100) * activeAm).toFixed(2)));
                        ...
                        return (
                          <>
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                              <div>
                                <h4 className="text-xs font-bold text-white uppercase tracking-wider">
                                  Plot Lengkung CSA (21 Station: 0 AP s.d 20 FP)
                                </h4>
                                <p className="text-[10px] text-slate-400 font-mono">
                                  Luas Midship Am = B x T x Cm = {activeAm.toFixed(2)} m² (B={Number(designData.breadth_m || 0).toFixed(2)}m, T={Number(designData.draft_m || 0).toFixed(3)}m, Cm={Number(designData.cm || 0.98).toFixed(2)})
                                </p>
                              </div>
                              <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-md text-emerald-400 bg-emerald-500/10 border border-emerald-500/20">
                                📖 NSP Referensi Wageningen
                              </span>
                            </div>
                            <div className="w-full h-48 bg-slate-900/60 rounded-xl relative overflow-hidden border border-slate-800/80 flex items-center justify-center shadow">
                              <svg className="w-full h-full px-4 py-2" viewBox="0 0 100 20" preserveAspectRatio="none">
                                {/* Vertical Station Lines 0 to 20 */}
                                {Array.from({ length: 21 }).map((_, i) => {
                                  const x = (i / 20) * 100;
                                  return <line key={i} x1={x} y1="0" x2={x} y2="20" stroke="#1e293b" strokeWidth="0.15" strokeDasharray="0.5,0.5" />;
                                })}
                                {/* Gridlines */}
                                <line x1="0" y1="18" x2="100" y2="18" stroke="#334155" strokeWidth="0.3" />
                                <line x1="50" y1="0" x2="50" y2="20" stroke="#334155" strokeWidth="0.3" />
                                {/* Curve */}
                                <path
                                  d={`M 0,18 ${csaOrds.map((val: number, idx: number) => {
                                    const x = (idx / 20) * 100;
                                    const max_val = Math.max(...csaOrds) || 1;
                                    const y = 18 - (val / max_val) * 16;
                                    return `L ${x},${y}`;
                                  }).join(" ")} L 100,18`}
                                ...
                                />
                                {/* Station Markers */}
                                {csaOrds.map((val: number, idx: number) => {
                                  const x = (idx / 20) * 100;
                                  const max_val = Math.max(...csaOrds) || 1;
                                  const y = 18 - (val / max_val) * 16;
                                  return (
                                    <g key={idx}>
                                      <title>{`Station ${idx}: Luas = ${val.toFixed(2)} m²`}</title>
                                      <circle cx={x} cy={y} r="0.5" fill="#38bdf8" />
                                    </g>
                                  );
                                })}
                              </svg>
                    ...
                    {/* DWL Half Breadth Chart */}
                    <div className="space-y-2">
                      <h4 className="text-xs font-bold text-white uppercase tracking-wider">Plot Garis Air DWL (Setengah Lebar)</h4>
                      <div className="w-full h-40 bg-slate-900/60 rounded-xl relative overflow-hidden border border-slate-800/80 flex items-center justify-center shadow">
                        {designData.geometry?.dwl_ordinates ? (
                          <svg className="w-full h-full px-4 py-2" viewBox="0 0 100 20" preserveAspectRatio="none">
                            <line x1="0" y1="2" x2="100" y2="2" stroke="#334155" strokeWidth="0.3" />
                            <line x1="50" y1="0" x2="50" y2="20" stroke="#334155" strokeWidth="0.3" />
                            <path
                              d={`M 0,2 ${designData.geometry.dwl_ordinates.map((val: number, idx: number) => {
                                const x = (idx / 20) * 100;
                                const max_val = Math.max(...designData.geometry.dwl_ordinates) || 1;
                                const y = 2 + (val / max_val) * 16;
                                return `L ${x},${y}`;
                              }).join(" ")} L 100,2`}
                              ...
                              strokeWidth="0.9"
                            />
                            {designData.geometry.dwl_ordinates.map((val: number, idx: number) => {
                              const x = (idx / 20) * 100;
                              const max_val = Math.max(...designData.geometry.dwl_ordinates) || 1;
                              const y = 2 + (val / max_val) * 16;
                              return <circle key={idx} cx={x} cy={y} r="0.5" fill="#34d399" />;
                            })}
                          </svg>
                        ) : (
                          <span className="text-xs text-slate-500 font-mono">Tidak ada data geometri.</span>
                        )}
                      </div>
                    </div>
                  </div>
